...
import r3n_runner as core

PREREG = 'TEACHER_ECONOMIC_R3N_FAILED_EXTREME_REVERSION_PREREG_V1.json'
...
def evaluate(frames: dict[str, pd.DataFrame], outdir: Path) -> dict:
    ...
    print('R3N_2023_FIXED', 'candidates', len(cand23), json.dumps(sm23, default=str), flush=True)

    result = {
        'format': 'VARDHANI_R3_TEACHER_ECONOMIC_R3N_DEVELOPMENT_RESULT_V1',
    ...
    if not sm23['pass']:
        result['status'] = 'RETIRED_NO_2023_GATE_PASS__2024_NOT_OPENED_FOR_R3N'
        return result

    tr23.to_csv(outdir / 'R3N_2023_FROZEN_TRADES.csv', index=False)
    cand24 = core.build_year_candidates(state, 2024)
    tr24 = core.simulate(cand24)
    sm24 = core.summarize(tr24)
    print('R3N_2024_FROZEN_AUDIT', 'candidates', len(cand24), json.dumps(sm24, default=str), flush=True)
    result['audit_2024_opened'] = True
    result['candidate_count_2024'] = int(len(cand24))
    result['audit_2024'] = sm24
    tr24.to_csv(outdir / 'R3N_2024_AUDIT_TRADES.csv', index=False)
    result['status'] = (
...
def main() -> int:
    ...
    ap.add_argument('--work', type=Path, default=Path('r3n_exact_cpu_work'))
    args = ap.parse_args()
    args.work.mkdir(parents=True, exist_ok=True)

    prereg = json.loads((HERE / PREREG).read_text())
    if prereg.get('status') != 'FROZEN_BEFORE_ANY_R3N_OUTPUT':
        raise RuntimeError('FAIL_CLOSED: R3N preregistration not frozen')
    sig = prereg['signal']
    hard = prereg['hard_boundaries']
    if tuple(sig['checkpoints_ist']) != core.CHECKPOINTS:
        raise RuntimeError('FAIL_CLOSED: R3N checkpoints mismatch')
    if int(sig['adaptive_history'].split()[1]) != core.LOOKBACK_DATES:
        raise RuntimeError('FAIL_CLOSED: R3N history mismatch')
    if int(sig['minimum_history_dates']) != core.MIN_HISTORY:
        raise RuntimeError('FAIL_CLOSED: R3N minimum-history mismatch')
    if float(sig['wick_quantile']) != core.WICK_Q:
        raise RuntimeError('FAIL_CLOSED: R3N wick quantile mismatch')
    if prereg['execution']['maximum_trades_per_date'] != core.MAX_TRADES_PER_DATE:
        raise RuntimeError('FAIL_CLOSED: R3N max-trades mismatch')
    if hard['teacher_v2_changed'] or hard['student_optimizer_steps'] != 0:
        raise RuntimeError('FAIL_CLOSED: R3N teacher/student boundary mismatch')
    if hard['gpu_allowed'] or hard['real_orders'] or hard['2025_used'] or hard['2026_used'] or hard['economic_edge_claimed']:
        raise RuntimeError('FAIL_CLOSED: R3N hard boundary mismatch')

    frames, audit = source.reconstruct_frames(args.work / 'exact_members')
    (args.work / 'R3N_EXACT_MEMBER_RECONSTRUCTION_AUDIT_V1.json').write_text(
        json.dumps({'status': 'PASS_ALL_108_BYTE_IDENTICAL', 'members': audit}, indent=2) + '\n'
    )
    result = evaluate(frames, args.work)
    (args.work / 'TEACHER_ECONOMIC_R3N_DEVELOPMENT_RESULT_V1.json').write_text(
        json.dumps(result, indent=2, default=str) + '\n'
    )
    print('R3N_FINAL_RESULT', json.dumps(result, default=str), flush=True)
    return 0
