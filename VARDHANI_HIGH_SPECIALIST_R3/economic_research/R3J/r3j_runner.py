...
CHECKPOINT_STRINGS = ('09:45','09:50','09:55','10:00','10:05','10:10','10:15','10:20','10:25','10:30')
FIXED_Q = 0.70
LOOKBACK_DATES = 126
MIN_HISTORY = 120
HORIZONS = (60,120)
...
def build_state(frames: dict[str,pd.DataFrame]) -> pd.DataFrame:
    ...
    if set(x.year.unique()) != {2022,2023,2024}:
        raise RuntimeError('FAIL_CLOSED: R3J state year boundary invalid')
    return x
...
def checkpoint_frame(x: pd.DataFrame) -> pd.DataFrame:
    ...
    c['gap_thr'] = np.nan; c['mom_thr'] = np.nan; c['trend_med'] = np.nan
...
def build_year_candidates(x: pd.DataFrame, year: int) -> pd.DataFrame:
    if year not in (2023,2024):
        raise RuntimeError('FAIL_CLOSED: R3J year must be 2023 or 2024')
    ...
    c = c.sort_values('timestamp').drop_duplicates('date', keep='first').copy()
