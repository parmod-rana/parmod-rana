...
import r3f_runner as core

PREREG = 'TEACHER_ECONOMIC_R3F_RECENCY_CONSENSUS_ROUTER_PREREG_V1.json'
...
def evaluate(frames: dict[str, pd.DataFrame], outdir: Path) -> dict:
    ...
    for q in core.Q_GRID:
        tr = core.simulate(cand23, q)
        sm = core.summarize(tr)
        grid[str(q)] = sm
        trade_by_q[q] = tr
        print('R3F_2023_Q', q, json.dumps(sm, default=str), flush=True)
        if sm['pass']:
            passing.append(q)
    ...
    result = {
        'format': 'VARDHANI_R3_TEACHER_ECONOMIC_R3F_DEVELOPMENT_RESULT_V1',
        ...
        'method': {
            'fast_window_dates': core.FAST_DATES,
            'slow_window_dates': core.SLOW_DATES,
            'fast_half_life': core.FAST_HALF_LIFE,
            'slow_half_life': core.SLOW_HALF_LIFE,
            'horizons': list(core.HORIZONS),
            'q_grid': list(core.Q_GRID),
            'consensus': 'both raw side scores > 0; percentile=min(fast_pct,slow_pct)',
        },
        'boundary': {
    ...
    if selected is None:
        result['status'] = 'RETIRED_NO_2023_CALIBRATION_PASS__2024_NOT_OPENED_FOR_R3F'
    else:
        # This is the first R3F use of 2024 outcomes and occurs only after Q is frozen.
        ...
        trade_by_q[selected].to_csv(outdir / 'R3F_2023_SELECTED_TRADES.csv', index=False)
        tr24.to_csv(outdir / 'R3F_2024_AUDIT_TRADES.csv', index=False)
    return result


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument('--work', type=Path, default=Path('r3f_exact_cpu_work'))
    a = ap.parse_args()
    a.work.mkdir(parents=True, exist_ok=True)

    prereg = json.loads((HERE / PREREG).read_text())
    if prereg.get('status') != 'FROZEN_BEFORE_ANY_R3F_MODEL_OUTPUT':
        raise RuntimeError('FAIL_CLOSED: R3F preregistration not frozen')
    hard = prereg['hard_boundaries']
    if hard['student_optimizer_steps'] != 0 or hard['2025_used'] or hard['2026_used'] or hard['gpu_allowed']:
        raise RuntimeError('FAIL_CLOSED: R3F hard boundary mismatch')

    frames, audit = source.reconstruct_frames(a.work / 'exact_members')
    (a.work / 'R3F_EXACT_MEMBER_RECONSTRUCTION_AUDIT_V1.json').write_text(
        json.dumps({'status': 'PASS_ALL_108_BYTE_IDENTICAL', 'members': audit}, indent=2) + '\n'
    )
    result = evaluate(frames, a.work)
    (a.work / 'TEACHER_ECONOMIC_R3F_DEVELOPMENT_RESULT_V1.json').write_text(
        json.dumps(result, indent=2, default=str) + '\n'
    )
    print('R3F_FINAL_RESULT', json.dumps(result, default=str), flush=True)
    return 0
