...
import r3p_runner as core

PREREG = 'TEACHER_ECONOMIC_R3P_VOLATILITY_EXPANSION_TREND_PREREG_V1.json'
...
def evaluate(frames: dict[str, pd.DataFrame], outdir: Path) -> dict:
    ...
    print('R3P_2023_FIXED', 'candidates', len(cand23), json.dumps(sm23, default=str), flush=True)
    result = {
        'format': 'VARDHANI_R3_TEACHER_ECONOMIC_R3P_DEVELOPMENT_RESULT_V1',
        ...
        'method': {
            ...
            'rv15_quantile': core.RV15_Q,
            'momentum15_quantile': core.MOM15_Q,
    ...
    if not sm23['pass']:
        result['status'] = 'RETIRED_NO_2023_GATE_PASS__2024_NOT_OPENED_FOR_R3P'
        return result

    tr23.to_csv(outdir / 'R3P_2023_FROZEN_TRADES.csv', index=False)
    cand24 = core.build_year_candidates(state, 2024)
    tr24 = core.simulate(cand24)
    sm24 = core.summarize(tr24)
    print('R3P_2024_FROZEN_AUDIT', 'candidates', len(cand24), json.dumps(sm24, default=str), flush=True)
    result['audit_2024_opened'] = True
    result['candidate_count_2024'] = int(len(cand24))
    result['audit_2024'] = sm24
    tr24.to_csv(outdir / 'R3P_2024_AUDIT_TRADES.csv', index=False)
    result['status'] = (
...
def main() -> int:
    ...
    ap.add_argument('--work', type=Path, default=Path('r3p_exact_cpu_work'))
    args = ap.parse_args()
    args.work.mkdir(parents=True, exist_ok=True)

    prereg = json.loads((HERE / PREREG).read_text())
    if prereg.get('status') != 'FROZEN_BEFORE_ANY_R3P_OUTPUT':
        raise RuntimeError('FAIL_CLOSED: R3P preregistration not frozen')
    sig = prereg['signal']
    hard = prereg['hard_boundaries']
    if tuple(sig['checkpoints_ist']) != core.CHECKPOINTS:
        raise RuntimeError('FAIL_CLOSED: R3P checkpoints mismatch')
    if int(sig['adaptive_history'].split()[1]) != core.LOOKBACK_DATES:
        raise RuntimeError('FAIL_CLOSED: R3P history mismatch')
    if int(sig['minimum_history_dates']) != core.MIN_HISTORY:
        raise RuntimeError('FAIL_CLOSED: R3P minimum-history mismatch')
    if float(sig['rv15_quantile']) != core.RV15_Q:
        raise RuntimeError('FAIL_CLOSED: R3P rv15 quantile mismatch')
    if float(sig['momentum15_quantile']) != core.MOM15_Q:
        raise RuntimeError('FAIL_CLOSED: R3P momentum quantile mismatch')
    if prereg['execution']['maximum_trades_per_date'] != core.MAX_TRADES_PER_DATE:
        raise RuntimeError('FAIL_CLOSED: R3P max-trades mismatch')
    if prereg['horizon_router']['if_both_long_horizon_conditions_true'] != 60 or prereg['horizon_router']['otherwise'] != 30:
        raise RuntimeError('FAIL_CLOSED: R3P horizon router mismatch')
    if hard['teacher_v2_changed'] or hard['student_optimizer_steps'] != 0:
        raise RuntimeError('FAIL_CLOSED: R3P teacher/student boundary mismatch')
    if hard['gpu_allowed'] or hard['real_orders'] or hard['2025_used'] or hard['2026_used'] or hard['economic_edge_claimed']:
        raise RuntimeError('FAIL_CLOSED: R3P hard boundary mismatch')

    frames, audit = source.reconstruct_frames(args.work / 'exact_members')
    (args.work / 'R3P_EXACT_MEMBER_RECONSTRUCTION_AUDIT_V1.json').write_text(
        json.dumps({'status': 'PASS_ALL_108_BYTE_IDENTICAL', 'members': audit}, indent=2) + '\n'
    )
    result = evaluate(frames, args.work)
    (args.work / 'TEACHER_ECONOMIC_R3P_DEVELOPMENT_RESULT_V1.json').write_text(
        json.dumps(result, indent=2, default=str) + '\n'
    )
    print('R3P_FINAL_RESULT', json.dumps(result, default=str), flush=True)
    return 0
